...
async function main() {
  ...
  const erc20contract = await ERC20.deploy(NAME, SYMBOL, DECIMALS, UN_BILLON, UN_BILLON);
  const FNFT = await ethers.getContractFactory('FNFT');
  const instance = await FNFT.deploy('', erc20contract.address);

  // console.log("El contrato ERC20 fue desplegado con éxito en la dirección: " + erc20contract.address);
