    ...
    ReentrancyGuard
{
    ...
    function _saveAttributes(Attributes memory _attributes) internal {
        ...
        uint256 interestRate = GlobalMarker(
            registry.getContractAddress("GlobalMarker")
        ).getInterestRate();
        uint256 lifeBasedOnMult = multipliers[_attributes.multiplier][
            currMarker
        ];
    ...
    function withdrawLockedSavings(
        uint256 _id,
        uint256 _interest
    ) external nonReentrant {
        require(
        ...
        require(
            block.timestamp >=
                attributes[_id].timeCreated + attributes[_id].cfaLife,
            "LockedSavings: CFA is still locked"
        );

        BBToken(registry.getContractAddress("BbToken")).burn(
            attributes[_id].principal
        );
        BBToken(registry.getContractAddress("BbToken")).mint(
            msg.sender,
            _interest
        );
    ...
    function safeTransferFrom(
        ...
        require(
            attributes[id].timeCreated + attributes[id].cfaLife >=
                block.timestamp,
            "LockedSavings: CFA is still locked"
        );
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                attributes[ids[i]].timeCreated + attributes[ids[i]].cfaLife >=
                    block.timestamp,
                "LockedSavings: CFA is still locked"
            );
        }
