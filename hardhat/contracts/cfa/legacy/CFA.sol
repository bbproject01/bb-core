...
contract FNFT is ERC1155 {
     ...
     */
    function mint(
        uint256 _originalTerm,
        uint256 _maximumReduction,
        uint256 _amount
    ) public {
        require(
        ...
        idToFNFTMetadata[id] = FNFTMetadata(
            false,
            0,
            _originalTerm,
            block.timestamp,
     ...
     */
    function getInfoFNFTMetadata(
        uint256 _id
    ) public view returns (FNFTMetadata memory) {
    ...
    function withDrawFNFT(uint256 _id) public {
        ...
        require(
            _tokenOwners[_id] == msg.sender,
            "FNFT: Solo el propietario del FNFT puede reclamar los tokens"
        );
        uint256 timePass = idToFNFTMetadata[_id].createDate +
            idToFNFTMetadata[_id].originalTerm *
            30 *
            24 *
            60 *
            60; // 30 days, 24 hours, 60 minutes, 60 seconds
        require(
            block.timestamp >= timePass,
    ...
    function createLock(uint _id) public {
        require(
            _tokenOwners[_id] == msg.sender,
            "Only the owner can lock the token"
        );
        require(
            idToFNFTMetadata[_id].blockDate == 0,
            "Lock already exists for this token"
        );
